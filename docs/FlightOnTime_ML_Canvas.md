...
For each production variant, the winning configuration is refit on the last fold's training and validation windows together - the widest and most recent data available - and its calibration and decision threshold are then fixed on that fold's test window, which the refit never saw. The model benefits from every record up to that point, while the threshold is still chosen only on data it never trained on. The final model is calibrated so that the probabilities it reports can be read properly. Only these final models are released - that is, registered in the model registry and served by the API.
